...
async def create_comment(
    ...
    comment = await crud_comment.create_comment(db=db, data=data, current_user=current_user)

    return comment

@router.get('/comment/get_all_comment', response_model=list[comment.ResponseComment])
async def get_all_comment_task(
    task_id: int,
    current_user: Annotated[user.User, Security(deps.get_current_active_user, scopes=["me"])],
    db: Session = Depends(deps.get_db)
):
    comments = await crud_comment.get_all_comment_of_task(db,task_id)
...
async def comment_delete(
    ...
    comments = await crud_comment.delete_comment(db,comment_id, current_user)
